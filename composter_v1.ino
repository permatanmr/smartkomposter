...
const char* airLabel(int ppm){
 ...
 if(ppm <  400) return "MODERATE";
 if(ppm <  600) return "UNHEALTHY";
 if(ppm <  800) return "DANGEROUS";
 return           "HAZARDOUS";
}
...
void pageDHT(float t,float h,
 ...
 oled.setTextSize(1);
 oled.setCursor(0,40);
 oled.print("H:");oled.print(h,1);
 oled.print("%");
 drawBar(0,50,128,6,h);
 oled.setCursor(70,40);
 oled.print("HI:");oled.print(hi,1);
 oled.display();
}
...
void pageGas(int ppm2,int ppm7){
 ...
 oled.setTextSize(1);
 oled.setCursor(0,18);
 oled.print("LPG:");oled.print(ppm2);
 oled.print(g2?" !":" OK");
 drawBar(0,26,128,6,
   constrain(ppm2/10,0,100));
 oled.setCursor(0,36);
 oled.print("CO: ");oled.print(ppm7);
 oled.print(g7?" !":" OK");
 drawBar(0,44,128,6,
...
void pageSummary(
 ...
 oled.setTextSize(1);
 oled.setCursor(0,18);
 oled.print("T:");oled.print(t,1);
 oled.print(" H:");oled.print(h,0);
 oled.print("%");
 oled.setCursor(0,28);
 oled.print("AIR:");
...
void showAlert(
 ...
 if(a2>=300){
   oled.setCursor(6,y);
   oled.print("GAS LEAK (LPG)");y+=10;}
 if(a7>=400){
   oled.setCursor(6,y);
   oled.print("CO DETECTED");y+=10;}
 if(a3>=400){
   oled.setCursor(6,y);
   oled.print("ALCOHOL HIGH");y+=10;}
 if(a135>=500){
   oled.setCursor(6,y);
 ...
 switch(page){
   case 0:
     if(!isnan(temp))
       pageDHT(temp,hum,hi);
     break;
   case 1:pageAir(r135,p135);break;
   case 2:pageAlc(r3,p3);break;
   case 3:pageGas(p2,p7);break;
   case 4:
     pageSummary(temp,hum,
       p135,p3,p2,p7);break;
 }
